...
void read_inode(uint32_t start_block, ext2_super_block_t *sb,
                ext2_block_group_descriptor_t *bgdt, ext2_inode_t *inode,
  ...
  uint32_t lba =
      start_block + (inode_table_block + block_offset) * (block_size >> 9);
  uint8_t buffer[4096]; // Достаточно большой буфер
...
void read_bitmap(uint32_t start_block, uint32_t block_size,
                 ext2_block_group_descriptor_t *bgdt, uint8_t type,
                 uint8_t *buffer) {
...
void set_bit(uint32_t start_block, uint32_t block_size,
             ext2_block_group_descriptor_t *bgdt, uint8_t type, uint8_t *bitmap,
...
void read_dir(uint32_t start_block, ext2_super_block_t *sb,
              ext2_block_group_descriptor_t *bgdt, uint32_t inode_number,
              vfs_node_t *entries, size_t max_entries, size_t *count_out) {
  *count_out = 0;
